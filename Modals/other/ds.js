...
module.exports = {
  ...
  async submit({ client, channel, guild, member, interaction, db }) {
    ...
    if (reason && reason.length > 0) {
      embed.addFields([
        {
          name: "Denied by " + member.user.username,
          value: reason,
        },
      ]);
    } else {
      embed.addFields([
        {
          name: "​",
          value: "**Denied by " + member.user.username + "**",
        },
      ]);
    }
    let attachments = message.attachments;
    if (channeldb.denychannel != undefined) {
      await interaction.editReply({ content: " ", embeds:[embed], files:attachments, components:[] });
      if (channeldb.delete && channeldb.delete.includes("denied")) {
        await message.delete();
      }
      let denychannel = await guild.channels.fetch(channeldb.denychannel);
      denychannel.send({ content: " ", embeds:[embed], files:attachments });
    } else {
      await interaction.editReply({ content: " ", embeds:[embed], files:attachments, components:[] });
    }
    if (urlParams.get("message") != null) {
      ...
      if (channeldb.acceptchannel != undefined) {
        await message.edit({ content: " ", embeds: [embed], files: attachments, components: [] });
        if (channeldb.delete && channeldb.delete.includes("denied")) {
          await message.delete();
        }
      } else {
